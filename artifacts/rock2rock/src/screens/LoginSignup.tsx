...
function InputField({
  id, label, type, value, onChange, placeholder, icon: Icon, required, testId,
  showToggle, onToggle, showPassword,
}: {
  id: string; label: string; type: string; value: string;
  onChange: (v: string) => void; placeholder: string; icon: React.ElementType;
  required?: boolean; testId?: string; showToggle?: boolean;
...
export function LoginSignup() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPlayerProfile(data.id, data.username, data.region, data.score);
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/players/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: signupUsername.trim(), email: signupEmail.trim(), region: signupRegion, password: signupPassword }),
      });
      const data = await res.json();
        ...
        <div className="flex bg-black/40 rounded-xl p-1 mb-7 border border-white/10">
          {(["login", "signup"] as Tab[]).map((t) => (
            <button key={t} onClick={() => { setTab(t); setLoginError(""); setSignupError(""); }}
              className={`flex-1 py-2.5 rounded-lg text-sm font-bold tracking-widest uppercase transition-all ${tab === t ? "bg-purple-600 text-white shadow-[0_0_15px_rgba(168,85,247,0.5)]" : "text-white/40 hover:text-white/70"}`}>
              {t}
            </button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          {tab === "login" ? (
            <motion.form key="login"
              ...
              <InputField id="login-email" label="Gmail" type="email"
                value={loginEmail} onChange={setLoginEmail}
                placeholder="you@gmail.com" icon={Mail} required testId="input-login-email" />
              <InputField id="login-password" label="Password" type="password"
                value={loginPassword} onChange={setLoginPassword}
                placeholder="••••••••" icon={Lock} required testId="input-login-password"
              ...
              <button type="submit" disabled={!loginEmail.trim() || !loginPassword || loginLoading}
                ...
                {loginLoading ? "Entering..." : "Enter Arena"}
              </button>
              <p className="text-center text-xs text-white/30 pt-1">
                No account?{" "}
                <button type="button" onClick={() => setTab("signup")} className="text-purple-400 hover:text-purple-300 underline transition-colors">Sign up</button>
              </p>
            </motion.form>
          ) : (
            <motion.form key="signup"
              ...
              <InputField id="signup-username" label="Username" type="text"
                value={signupUsername} onChange={setSignupUsername}
                placeholder="Your game alias" icon={User} required testId="input-signup-username" />
              <InputField id="signup-email" label="Gmail" type="email"
                value={signupEmail} onChange={setSignupEmail}
                placeholder="you@gmail.com" icon={Mail} required testId="input-signup-email" />
              <div className="space-y-1.5">
                <label htmlFor="signup-region" className="text-xs font-semibold text-purple-300 tracking-widest uppercase">Region</label>
                <div className="relative flex items-center">
                  <Globe size={15} className="absolute left-3.5 text-purple-400/60 pointer-events-none" />
                  <select id="signup-region" value={signupRegion} onChange={(e) => setSignupRegion(e.target.value)}
              ...
              <InputField id="signup-password" label="Password" type="password"
                value={signupPassword} onChange={setSignupPassword}
                placeholder="Min. 6 characters" icon={Lock} required testId="input-signup-password"
                showToggle onToggle={() => setShowSignupPw(v => !v)} showPassword={showSignupPw} />
              {signupError && <p className="text-red-400 text-xs text-center font-semibold">{signupError}</p>}
              <button type="submit" disabled={!signupUsername.trim() || !signupEmail.trim() || !signupPassword || signupLoading}
                className="w-full py-3.5 mt-2 bg-purple-600 hover:bg-purple-500 disabled:bg-purple-900 disabled:opacity-50 text-white font-bold rounded-xl shadow-[0_0_20px_rgba(168,85,247,0.4)] hover:shadow-[0_0_30px_rgba(168,85,247,0.7)] transition-all uppercase tracking-widest text-sm disabled:cursor-not-allowed"
              ...
              <p className="text-center text-xs text-white/30 pt-1">
                ...
                <button type="button" onClick={() => setTab("login")} className="text-purple-400 hover:text-purple-300 underline transition-colors">Log in</button>
              </p>
            </motion.form>
          )}
        </AnimatePresence>
